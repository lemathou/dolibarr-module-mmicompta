...
/**
 * \file    test/phpunit/MMIComptaFunctionalTest.php
 * \ingroup mmicompta
 ...
 *
 * @package Testmmicompta
 */
class MMIComptaFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modMMICompta')->click();
		} else {
			// Disable the module
			$this->byHref('modMMICompta')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/mmicompta/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('mmicompta/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/mmicompta/admin/about.php');
		$this->authenticate();
		return $this->assertContains('mmicompta/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('mmicomptawidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMMICompta_MMIComptaTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMMICompta_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
